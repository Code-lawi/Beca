    ...
    if (empty($name) || empty($price) || empty($description)) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($file['type'], $allowed_types)) {
                ...
                $target_path = PRODUCT_IMG_PATH . $new_image;
                
                if (move_uploaded_file($file['tmp_name'], $target_path)) {
                    // Delete old image
                    if (!empty($product['image']) && file_exists(PRODUCT_IMG_PATH . $product['image'])) {
                        unlink(PRODUCT_IMG_PATH . $product['image']);
                    }
        ...
        // Update database
        if (empty($error)) {
            $stmt = $conn->prepare("UPDATE products SET name = ?, price = ?, description = ?, image = ?, stock = ?, status = ? WHERE id = ?");
            $stmt->bind_param("sdssiii", $name, $price, $description, $image_name, $stock, $status, $id);
...
<head>
    ...
    <title>Edit Product - SCI-CALC Admin</title>
    ...
    <style>
        ...
        * {
        ...
        .form-container .form-title .product-id {
            font-size: 12px;
            color: var(--gray);
            font-weight: 400;
            background: var(--bg);
        ...
        .current-image-wrapper .image-info {
            flex: 1;
        }

        .current-image-wrapper .image-info .label {
            font-size: 12px;
            color: var(--gray);
            font-weight: 500;
        }

        .current-image-wrapper .image-info .filename {
            font-size: 13px;
            font-weight: 500;
            color: var(--dark);
        }
...
<div class="main-content">
    ...
    <div class="form-container">
        <div class="form-title">
            <span><i class="fas fa-box" style="color:var(--primary);"></i> Edit Product Information</span>
            <span class="product-id">
                <i class="fas fa-clock"></i> Last updated: <?php echo date('M d, Y H:i', strtotime($product['created_at'])); ?>
            </span>
        </div>
        ...
        <form method="POST" enctype="multipart/form-data" id="productForm" novalidate>
            ...
            <div class="form-group">
                <label>
                    <span class="label-icon">📷</span> Product Image
                </label>
                
                <div class="current-image-wrapper">
                    <img src="<?php echo '../uploads/products/' . $product['image']; ?>" alt="Current image" 
                         onerror="this.src='data:image/svg+xml,%3Csvg xmlns=%22http://www.w3.org/2000/svg%22 width=%2270%22 height=%2270%22%3E%3Crect width=%2270%22 height=%2270%22 fill=%22%23f0f2f5%22/%3E%3Ctext x=%2216%22 y=%2242%22 font-size=%2224%22%3E📷%3C/text%3E%3C/svg%3E'">
                    <div class="image-info">
                        <div class="label">Current Image</div>
                        <div class="filename"><?php echo htmlspecialchars($product['image']); ?></div>
                    </div>
                </div>
            ...
            <div class="form-group">
                ...
                <div class="helper-text">
                    <i class="fas fa-info-circle"></i>
                    <span id="charCount"><?php echo strlen($product['name']); ?></span> characters
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="stock" id="productStock" 
                           placeholder="e.g., 10" value="<?php echo $product['stock']; ?>" min="0">
                    <div class="helper-text">
                        <i class="fas fa-info-circle"></i>
                        <?php if ($product['stock'] > 0): ?>
                            <span style="color:var(--success);">✓ <?php echo $product['stock']; ?> in stock</span>
                        <?php else: ?>
                            <span style="color:var(--danger);">⚠️ Out of stock</span>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <div class="helper-text">
                    <i class="fas fa-info-circle"></i>
                    <span id="descCharCount"><?php echo strlen($product['description']); ?></span> characters
                </div>
            </div>
            ...
            <div class="form-group">
                <div class="form-check">
                    <input type="checkbox" name="status" id="status" <?php echo $product['status'] ? 'checked' : ''; ?>>
                    <label for="status">
                        <i class="fas fa-eye" style="color:<?php echo $product['status'] ? 'var(--success)' : 'var(--gray)'; ?>;"></i>
                        Visible on homepage
                        <span class="check-sub">— <?php echo $product['status'] ? 'Currently visible to customers' : 'Currently hidden from customers'; ?></span>
                    </label>
                </div>
            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // 8. CONSOLE WELCOME
        // ========================================
        console.log('%c✏️ SCI-CALC Edit Product', 'font-size:24px;font-weight:800;color:#6C63FF;');
        console.log('%c📝 Editing: <?php echo htmlspecialchars($product['name']); ?>', 'font-size:14px;color:#2D3436;');
        console.log('%c💡 Tip: Ctrl+S to save, ESC to cancel', 'font-size:12px;color:#8899AA;');
        console.log('%c✅ Edit product page loaded successfully!', 'font-size:14px;color:#00C9A7;');
    });
</script>
